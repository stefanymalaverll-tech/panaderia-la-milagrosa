...
export default function ModalDetallesC({
  show,
  onClose,
  cajaSeleccionada,
  ordenesCaja = [],
  cargandoOrdenes,
  onEliminarOrden = null
}) {
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl max-w-3xl w-full shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-2xl">
          <div>
            <h3 className="text-lg font-bold text-slate-800">
              🧾 Desglose del Turno #{cajaSeleccionada?.id_caja}
            </h3>
        ...
        <div className="p-6 overflow-y-auto bg-slate-100/50 flex-1">
          {cargandoOrdenes ? (
            <div className="text-center py-10 font-bold text-slate-500 animate-pulse">
              Cargando facturas...
            </div>
                ...
                <div key={orden.id_orden} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                  ...
                  <div className="flex justify-between items-center border-b border-slate-100 pb-3 mb-3">
                    <div className="flex items-center gap-2">
                      <span className="bg-amber-100 text-amber-800 text-[10px] font-bold px-2 py-1 rounded-md uppercase">
                        Ticket #{orden.num_ticket}
                      </span>
                    ...
                    <div className="flex items-center gap-4 text-right">
                      <div>
                        <div className="text-sm font-extrabold text-slate-800">
                          Bs. {Number(orden.total_bs).toFixed(2)}
                        </div>
                        <div className="text-[10px] font-bold text-slate-500">
                          ${Number(orden.total_usd).toFixed(2)} USD
                        </div>
                      </div>
                        ...
                        <li key={idx} className="flex justify-between text-xs items-center">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium text-slate-700">
                              {detalle.producto?.icono_producto?.simbolo || '📦'} {detalle.producto?.nombre}
                            </span>
                          </div>
                          <span className="text-slate-600 font-semibold">
                            Bs. {Number(detalle.subtotal_bs || (detalle.subtotal_usd * (orden.total_bs / orden.total_usd))).toFixed(2)}
                          </span>
                        </li>
                      ))}
                    </ul>
                      ...
                      {orden.pago_orden?.map((pagoLinea, idx) => (
                        <li key={idx} className="flex justify-between text-[11px] items-center">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className={`font-medium ${pagoLinea.es_vuelto ? 'text-red-600' : 'text-slate-600'}`}>
                              {pagoLinea.pago?.nombre}{pagoLinea.es_vuelto && ' (Vuelto)'}
                            </span>
                            {pagoLinea.numero_referencia && (
                              <span className="text-slate-400">Ref: {pagoLinea.numero_referencia}</span>
                            )}
                          </div>
                          <span className={`font-semibold ${pagoLinea.es_vuelto ? 'text-red-600' : 'text-slate-700'}`}>
                            {pagoLinea.pago?.moneda === 'Bs' 
                              ? `Bs. ${Number(pagoLinea.monto_bs).toFixed(2)}` 
                              : `$${Number(pagoLinea.monto_usd).toFixed(2)} USD (Bs. ${Number(pagoLinea.monto_bs).toFixed(2)})`}
                          </span>
                        </li>
                      ))}
                    </ul>
        ...
        <div className="p-4 border-t border-slate-100 bg-white rounded-b-2xl flex justify-end">
          <button 
            onClick={onClose} 
            className="px-6 py-2 bg-slate-800 hover:bg-slate-900 text-white text-xs font-bold rounded-xl cursor-pointer shadow-sm transition-all"
          >
            Cerrar
          </button>
        </div>
