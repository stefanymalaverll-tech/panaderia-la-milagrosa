        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          {!esPanaderia ? (
            <div>
              ...
              <input 
                ...
                onChange={e => {
                  ...
                  if (val === '') { setProdEditando({...prodEditando, precio_inversion: ''}); return; }
                  const num = parseFloat(val);
                  if (!isNaN(num) && !val.toLowerCase().includes('e') && num <= 999999.99) {
                    setProdEditando({...prodEditando, precio_inversion: val});
                  }
                }}
            ...
            <div>
              ...
              <input type="text" disabled value="No Aplica" className="w-full bg-slate-100 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-500 mt-1 cursor-not-allowed" />
            </div>
          )}
          ...
          <div>
            ...
            <input 
              ...
              onChange={e => {
                const val = e.target.value;
                if (val === '') { setProdEditando({...prodEditando, precio_detal: ''}); return; }
                const num = parseFloat(val);
                if (!isNaN(num) && !val.toLowerCase().includes('e') && num <= 999999.99) {
          ...
          <div>
            ...
            <input 
              ...
              onChange={e => {
                const val = e.target.value;
                if (val === '') { setProdEditando({...prodEditando, precio_mayor: ''}); return; }
                const num = parseFloat(val);
                if (!isNaN(num) && !val.toLowerCase().includes('e') && num <= 999999.99) {
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {!esPanaderia ? (
              <div>
                ...
                <input 
                  ...
                  onChange={e => {
                    const val = e.target.value;
                    if (val === '') { setProdEditando({...prodEditando, stock: ''}); return; }
                    const num = parseFloat(val);
                    if (!isNaN(num) && !val.toLowerCase().includes('e') && num <= 999999.99) {
            ...
            <div>
              ...
              <input 
                ...
                onChange={e => {
                  const val = e.target.value;
                  if (val === '') { setProdEditando({...prodEditando, cant_min_mayor: ''}); return; }
                  const num = parseFloat(val);
                  if (!isNaN(num) && !val.toLowerCase().includes('e') && num <= 9999.99) {
